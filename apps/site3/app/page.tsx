...
import { SITE_COLORS, getProfiles, createProfile } from "@repo/shared";

export default function Site3Page() {
  const SITE_ID = 3;
  
  const [ownProfiles, setOwnProfiles] = useState<Array<any>>([]);
  const [sharedProfiles, setSharedProfiles] = useState<Array<any>>([]);
  ...
  useEffect(() => {
    const fetchProfiles = async () => {
      try {
        ...
        setOwnProfiles(profilesData.ownProfiles);
        setSharedProfiles(profilesData.sharedProfiles);
        setLoading(false);
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.shareWithSite1) sharedWith.push(1);
    if (formData.shareWithSite2) sharedWith.push(2);
    
    try {
      // Directly call the site's own API endpoint to create a profile
      // The backend will automatically assign the profile to the correct customer
              ...
              <div className="flex flex-col gap-2">
                <label className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    name="shareWithSite1"
                    checked={formData.shareWithSite1}
                    onChange={handleChange}
                  />
                  <span>Site 1</span>
                </label>
              ...
              <div className="space-y-4">
                {sharedProfiles.map(profile => (
                  <ProfileCard
                    ...
                    siteColor={SITE_COLORS[profile.customer.siteId as keyof typeof SITE_COLORS]}
                  />
                ))}
              </div>
            </>

...
export default function ProfileForm({ customers = [] }: ProfileFormProps) {
  const [formData, setFormData] = useState({
    name: "",
    age: "",
    customerId: "",
    sharedWith: [] as number[]
  });
  ...
  // Fetch customers if not provided via props
  useEffect(() => {
        ...
        try {
          ...
          if (response.ok && data.data) {
            ...
            if (data.data.length > 0) {
              setFormData(prev => ({
                ...prev,
                customerId: data.data[0].id.toString()
              }));
            }
          }
        } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setMessage({ type: "", text: "" });
      ...
      setFormData(prev => ({
        name: "",
        age: "",
        customerId: prev.customerId, // Keep the same customer selected
        sharedWith: []
      }));
